...
## 음악·효과음
- 효과음은 `sfx_gen.py`가 합성하고, 장면에서 실제로 일어나는 일에 맞춰 자동 배치됩니다:
  스티커 등장(뿅), 카운트다운(삐), 정답(딩), 알림(차임), 채팅 전송(톡), 달리기(발소리), 출발 호루라기·환호,
  슬롯 회전·정지·잭팟 팡파르, 색종이(빠밤), 반짝, 문 두드림·삐걱, 사다리 추적, 빗소리·바람·새소리 등 33종.
  사이드바에서 끄거나 볼륨 조절 가능. CLI는 `--no-sfx`, `--sfx-volume`.
